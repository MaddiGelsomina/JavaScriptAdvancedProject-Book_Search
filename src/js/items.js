...
export async function createBookItem(bookTitle, bookAuthors, bookId) {
  ...
  const title = createItem("h3", `${bookTitle}`, "card-header");
  bookItem.append(title);

  const authors = createItem("p", `${bookAuthors}`, "card-header");
  bookItem.append(authors);

  const expandButton = createItem(
    "button",
    `<svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="green" class="bi bi-arrow-down-circle" viewBox="0 0 16 16">
  ...
  const descriptionTitle = createItem("h4", "Description");
  description.append(descriptionTitle);
  desc = validateDescription(desc);
  const descriptionText = createItem("p", desc);
